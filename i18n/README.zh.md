...
| **1 · 语义** | 一个**嵌入**模型（`all-minilm` 45 MB · `paraphrase-multilingual` ~560 MB） | 按**含义**、跨语言搜索——recall@1 = **0.93**，recall@3 **1.00** |
...
由 [`eval/ygg_eval.py`](../eval/ygg_eval.py) 测得——35 个带标注的查询，排序权重仅在 *dev* 划分上调优，所以 **holdout 才是无偏的数字**（使用 `paraphrase-multilingual` 模型）：

| 搜索视图 | holdout recall@1 | recall@3 | 零依赖词法 |
| --- | --- | --- | --- |
| **在项目内**（真实路径，候选池 ~6） | **0.93** | **1.00** | 0.77 |
| **整个存储库**（无过滤，候选池 35） | 0.80 | **1.00** | 0.77 |

**两种视图下 recall@3 都 = 1.00**——用本地模型时，即使搜索整个存储库，正确的记忆也*总是*落在前 3 名里；在项目内则有 0.93 的概率排在第 1。零依赖词法模式已经能解决关键词和代码标识符类查询（1.00）。语料库较小（n=35），因此 [BENCHMARKS.md 中的完整拆解](../BENCHMARKS.md)给出了 95% 置信区间、候选池大小和分类得分——你也可以花一分钟左右自己重跑一遍：`python3 eval/ygg_eval.py --report`。
