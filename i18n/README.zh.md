...
`ygg install` 是一次性的引导式设置：它会安装一个常驻的后台服务，把 MCP 工具注册到它找到的每一个智能体宿主——**Claude Code、Codex、OpenCode**——并在硬件允许时推荐可选的本地模型（也可以选 `none`，保持零配置）。

<details>
<summary><b>OpenCode</b>——无需任何配置</summary>

先安装 [OpenCode](https://opencode.ai)，再运行 `ygg install`（如果 Yggdrasil 已经装好，就运行 `ygg redeploy`）——配置项会自动写好，并合并进已有的 `opencode.json`。用下面的命令确认：

```bash
opencode mcp list        # -> ✓ yggdrasil connected
```

在 Yggdrasil *之后*才装的 OpenCode？直接重新运行 `ygg install` 就行。

如果你更想手写配置，注意 OpenCode 的 schema 和 Claude 的同时有四处不同：服务器写在 `mcp` 下（不是 `mcpServers`），`type` 是必填项，`command` 是单个数组（不是 `command` + `args`），环境变量叫 `environment`（不是 `env`）——所以 Claude 的配置片段没法直接照搬：
...
配置里不需要放 token——引擎会自己读取那个权限为 0600 的 `~/.yggdrasil/token`。如果工具没出现，运行 `ygg doctor` 排查。
...
- 🧠 **持久**——决策、教训和项目状态跨会话留存。
- 🔌 **一个大脑，所有工具**——Claude Code、Codex、OpenCode 和任何 MCP 宿主共享同一份记忆。
...
| `nomic-embed-text` | 274 MB | 英文，质量更好（768d） |
| `mxbai-embed-large` | 670 MB | 英文，高质量（1024d） |
| `paraphrase-multilingual` | ~560 MB | 多语言（EN/RU + 50 种语言，768d） |
| `bge-m3` | 1.2 GB | 多语言，顶级质量（更重） |

**嵌入后端**——默认是 Ollama。如果想改用兼容 OpenAI 的
`/v1/embeddings` 服务器（llama.cpp 的 `llama-server --embeddings`、
OpenRouter、LM Studio、vLLM），设置 `embed_backend`：
...
密钥保存在 `~/.yggdrasil/embed_api_key`（0600）而不是
`config.json` 里，并以**文件路径**的形式传给守护进程——因此它不会出现
在 `ps`、launchd 的 plist 或 systemd 的 unit 里。`ygg config list` 会把它遮蔽掉。

用 `ygg doctor` 确认是否生效——dense 应该会显示你的模型名：
...
<details>
<summary><b>OpenRouter：两个容易踩坑的设置</b></summary>

**1. 用推理密钥，不要用配置密钥。**
[openrouter.ai/settings/provisioning-keys](https://openrouter.ai/settings/provisioning-keys)
里的密钥只能用来铸造其他密钥——用它调用嵌入接口，会得到一个令人困惑的
`401 User not found`。请改到
[openrouter.ai/settings/keys](https://openrouter.ai/settings/keys) 创建一个普通密钥。

注意 `GET /api/v1/models` 无论密钥是否有效都会返回 `200 OK`——它完全
不检查鉴权，所以没法靠它判断密钥能不能用。改查 `GET /api/v1/key`：
它会返回 `is_provisioning_key`，密钥不对时也会直接报错。

**2. 隐私设置会悄悄隐藏大多数模型。** 如果某个模型报 404 并提示
`All providers have been ignored`，模型本身没问题——是你的账号把提供
该模型的所有服务商都过滤掉了。到
[openrouter.ai/settings/privacy](https://openrouter.ai/settings/privacy) 修改设置。
同一个过滤器也是 `openai/text-embedding-3-*` 有时会返回 `403`
（服务商条款限制）的原因。

到
[openrouter.ai/models?output_modalities=embeddings](https://openrouter.ai/models?fmt=cards&output_modalities=embeddings)
（截稿时共 26 个模型）看看实际能用哪些。比较好用的：

| 模型 | 每百万 token 价格 |
| --- | --- |
| `nvidia/llama-nemotron-embed-vl-1b-v2:free` | **$0** |
| `perplexity/pplx-embed-v1-0.6b` | $0.004 |
| `intfloat/multilingual-e5-large` | $0.01——多语言 |
| `google/gemini-embedding-2` | $0.20 |

</details>

保持本地，在质量*和*隐私上依然更胜一筹：在 232 条记忆 / 110 条查询的语料库上，
本地的 `paraphrase-multilingual` 拿到 recall@1 **0.964**，高于免费托管模型的
**0.946**——而且你的记忆完全不会离开这台机器。托管后端该用在机器跑不动 Ollama
的时候，而不是为了追求精度。

在这里，更大的向量**不会**换来更高的精度——同一语料库上 `mxbai-embed-large`
（1024d）的得分是 0.809，`nomic-embed-text`（768d）是 0.818，二者的差距完全被
置信区间吞掉。真正影响得分的是模型是否覆盖*你用的*语言：这两个模型都只支持英文，
跨语言查询时会掉到 0.40–0.45，而多语言的默认模型能保持在 0.95。
