...
| **1 · セマンティック** | **埋め込み**モデル（`all-minilm` 45 MB · `paraphrase-multilingual` ~560 MB) | **意味**による、言語をまたいだ検索 — recall@1 = **0.94** |
...
[`eval/ygg_eval.py`](../eval/ygg_eval.py) で測定 — ラベル付きクエリ 35 件、dev/holdout 分割、recall@1:

| モード | recall@1 | パラフレーズ | 言語横断 (EN→RU) |
| --- | --- | --- | --- |
| 字句（既定) | 0.77 | 0.63 | 0.00 |
| 密 · `all-minilm` (45 MB) | 0.83 | 0.88 | 0.00 |
| 密 · `paraphrase-multilingual` (~560 MB) | **0.94** | 0.88 | **0.80** |

キーワードとコード識別子のクエリはどのモードでも 1.00、多言語モデルでは **recall@3 = 1.00** です。私たちの言葉を鵜呑みにしないでください — 約 1 分で再実行できます: `python3 eval/ygg_eval.py --mode lexical`（[BENCHMARKS.md](../BENCHMARKS.md) では `pip show` による依存ゼロの検証も行っています)。
