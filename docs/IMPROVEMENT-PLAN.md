...
16. ✅ **[DX] Retire demo-heritage defaults** — default identity is now `local`/`personal` (was `demo-user`/`yggdrasil-demo`); a version-guarded, backup-first SQL relabel auto-migrates existing memory once on engine start (also `ygg migrate [--dry-run]`), and pins the identity into `config.json` so no future default change can strand memory. ~15 hardcoded fallbacks routed through `ygg_config.user_id()/namespace()`; demo gates pin `ygg_config.DEMO_*` explicitly. Ships as 0.11.0 (breaking: sync key format). Tests in `tests/test_identity_migration.py`.
